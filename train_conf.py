...
def setup_ddp(rank, args):
    ...
    # Generate a random seed — for example, using the current time and rank
    seed = int(time.time()) % (2**32 - 1) + rank

    # If rank == 0, print it so it’s tracked
    if rank == 0:
...
def train(rank, args):
    ...
    print("Rank: ", rank)
    print("args.gpu_num: ", args.gpu_num)
    ...
    if args.ddp:
        ...
        train_loader = DataLoader(db, batch_size=args.batch, sampler=train_sampler, num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=True)
    else:
        train_loader = DataLoader(db, batch_size=args.batch, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=True)
                    ...
                    if args.eval:
                        ...
                        if args.evs:
                            ...
                            val_results, val_figures = eval_mvsec_evs(None, args, net.module if args.ddp else net, total_steps,
                                                                mvsec_datapath, args.val_split, return_figure=True, plot=True, rpg_eval=False,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--precomputed_inputs', action='store_true', help='use precomputed inputs (default: False)')
    parser.add_argument('--event_representation', type=str, default='voxel', help='event representation (default: stack)')
    parser.add_argument(
        '--use_softagg',
        type=lambda x: x.lower() == 'true',
        default=True,
