...
@torch.no_grad()
def run_voxel(voxeldir, cfg, network, viz=False, iterator=None, timing=False, H=480, W=640, viz_flow=False, scale=1.0, profile=False, **kwargs):

    #inistantiate the DEVO object
    print(f"Instantiating DEVO")
    slam = DEVO(cfg, network, evs=True, ht=H, wd=W, viz=viz, viz_flow=viz_flow,profiling=profile, **kwargs)
    
    print(f"Running DEVO on {voxeldir}")
...
def log_results(data, hyperparam, all_results, results_dict_scene, figures, 
                ...
                calib1_eds=None, camID_tumvie=None, outdir=None, expname="", max_diff_sec=0.01, save_csv=False, cfg=None):
    ...
    if rpg_eval:
        ...
        if cfg is not None and save_csv is not False:
            with open("results_try.csv", "a") as f:
                    f.write(
