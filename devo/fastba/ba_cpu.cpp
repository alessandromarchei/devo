    ...
    torch::Tensor update)
{
  ...
  for (int i=0; i < t1 - t0; i++) {
    ...
    std::cout << "Pose " << t << ": " << t0[0] << " " << t0[1] << " " << t0[2] << " "
    ...
    std::cout << xi[0] << " " << xi[1] << " " << xi[2] << " "
      << xi[3] << " " << xi[4] << " " << xi[5] << std::endl;
    ...
    torch::Tensor u, const int t0)
{
  ...
  for (int n=0; n < ii.size(0); n++) {
    ...
    std::cout << "Computing residuals for point " << n << ": "
    {
      ...
      for (int i=0; i<6; i++) {
        if (ix >= 0)
          E_accessor[6*ix+i][6*ix+6*j] += w * Ji[i] * Jz;
        if (jx >= 0)
          E_accessor[6*jx+i][6*jx+6*k] += w * Jj[i] * Jz;
      }
...

    {
      ...
      for (int i=0; i<6; i++) {
        if (ix >= 0)
          E_accessor[6*ix+i][k] -= w * Ji[i] * Jz;
        if (jx >= 0)
          E_accessor[6*jx+i][k] += w * Jj[i] * Jz;
      }
