...
void save_tensor_as_c_pair(const at::Tensor& tensor, const std::string& name, const std::string& base_filename, bool append = true) {
    // Paths
    std::string header_path = base_filename + ".h";
    std::string source_path = base_filename + ".c";

    // Show tensor shape
    std::cout << "[INFO] Saving tensor " << name << " with size: " << tensor.sizes() << std::endl;

    // Move to CPU and flatten
    at::Tensor t_cpu = tensor.contiguous().to(torch::kCPU).flatten();
    const float* data_ptr = t_cpu.data_ptr<float>();
    int64_t n = t_cpu.numel();

    // Open files
    std::ofstream h(header_path, append ? std::ios::app : std::ios::trunc);
    std::ofstream c(source_path, append ? std::ios::app : std::ios::trunc);

    if (!h.is_open() || !c.is_open()) {
        std::cerr << "[ERROR] Could not open header or source file." << std::endl;
        return;
    }

    // Header guard
    if (!append) {
        h << "// Auto-generated header for tensor declarations\n\n";
        h << "#pragma once\n";
        h << "#include <stdint.h>\n\n";
        c << "// Auto-generated source file for tensor values\n\n";
        c << "#include \"" << base_filename << ".h\"\n\n";
    }

    // Write extern in .h
    h << "extern float " << name << "[" << n << "];\n";

    // Write data in .c
    c << "float " << name << "[" << n << "] = {\n    ";
    c << std::fixed << std::setprecision(10);
    ...
    torch::Tensor update)
{
  ...
  //std::cout << "Number of poses: " << t1 - t0 << std::endl;
  //std::cout << "Updating poses..." << std::endl;
  for (int i=0; i < t1 - t0; i++) {

    //printf("Updating pose %d/%d\n", i, t1 - t0);
    const float t = t0 + i;
    ...
    float xi[6] = {
      ...
      //printf("Pose %d: t = [%f, %f, %f], q = [%f, %f, %f, %f]\n", i, t0[0], t0[1], t0[2], q0[0], q0[1], q0[2], q0[3]);

      //printf("Update value  %d: xi = [%f, %f, %f, %f, %f, %f]\n", i, xi[0], xi[1], xi[2], xi[3], xi[4], xi[5]);
    ...
    //printf("Updated pose %d: t = [%f, %f, %f], q = [%f, %f, %f, %f]\n", i, t1[0], t1[1], t1[2], q1[0], q1[1], q1[2], q1[3]);
  }
}
    ...
    torch::Tensor update)
{
    ...
    auto update_acc = update.accessor<float, 1>();
        //printf("Starting the update of the depths of the patches\n");

    int p = patches.size(2);
    for (int n = 0; n < index.size(0); n++) {
        int64_t ix = index_acc[n];
        //printf("Updating patch %d/%d\n", n, index.size(0));
        ...
        //printf("Update value: %f\n", update_acc[n]);

        d = d + update_acc[n];
        d = (d > 20.0f) ? 1.0f : d;
        d = std::max(d, 1e-4f);

        //printf("Updated depth: %f\n\n", d);
    ...
    torch::Tensor u, const int t0)
{
  ...
  //count number of edges with weight > 0.1
  int count = 0;
  for (int n=0; n < ii.size(0); n++) {
    if (weight_accessor[n][0] >= 1e-1 && weight_accessor[n][1] >= 1e-1) {
      count++;
    }
  }

  printf("Number of edges with weight >= 0.1: %d/%d\n", count, ii.size(0));

  for (int n=0; n < ii.size(0); n++) {
    //std::cout << "Processing point " << n << ": " << ii_accessor[n] << " " << jj_accessor[n] << " " << kk_accessor[n] << std::endl;
    if (weight_accessor[n][0] < 1e-1 || weight_accessor[n][1] < 1e-1) {
      // Skip this edge if the weight is too low
      continue;
    }
    ...
    const int t0, const int t1, const int iterations)
{
  ...
  std::cout << "CPU BA N poses : " << t1 - t0 << std::endl;
  ...
  std::cout << "BA CPU: " << N << " poses, " << M << " unique patches, "
            << num << " edges" << std::endl;  
  for (int itr=0; itr < iterations; itr++) {
    ...
    reprojection_residuals_and_hessian(
      ...
      //save into a txt file the B, E, C, v, u tensors, flattened by rows, and suitable for a .h file
    // S outaveput matrices as golden .h arrays
    std::string out_path = "ba_golden_matrix_fp32_" + std::to_string(itr);
    std::string out_path_h = out_path + ".h";
    std::string out_path_c = out_path + ".c";

    // Clear the file first
    std::ofstream clear_h(out_path_h, std::ios::trunc);
    std::ofstream clear_c(out_path_c, std::ios::trunc);
    
    clear_h.close();
    clear_c.close();

    // Save each tensor
    save_tensor_as_c_pair(B, "B_golden", out_path, false);
    save_tensor_as_c_pair(E, "E_golden", out_path);
    save_tensor_as_c_pair(C, "C_golden", out_path);
    save_tensor_as_c_pair(v, "v_golden", out_path);
    save_tensor_as_c_pair(u, "u_golden", out_path);
    ...
    save_tensor_as_c_pair(Q, "Q_golden", out_path);
    ...
    if (t1 - t0 == 0) {
      ...
      save_tensor_as_c_pair(EQ, "EQ_golden", out_path);
      ...
      save_tensor_as_c_pair(S, "S_golden", out_path);
      save_tensor_as_c_pair(y, "y_golden", out_path);

      //std::cout << "Cholesky decomposition..." << std::endl;
      torch::Tensor U = torch::linalg::cholesky(S);
      save_tensor_as_c_pair(U, "L_golden", out_path);
      torch::Tensor dX = torch::cholesky_solve(y, U);
      torch::Tensor dZ = Qt * (u - torch::matmul(Et, dX));

      save_tensor_as_c_pair(dX, "dX_golden", out_path);
      save_tensor_as_c_pair(dZ, "dZ_golden", out_path);
      ...
      at::Tensor patches_center = torch::empty({patches.size(0), 3}, patches.options());

      auto patches_acc = patches.accessor<float, 4>();
      auto center_acc = patches_center.accessor<float, 2>();

      for (int64_t n = 0; n < patches.size(0); ++n) {
          float x = patches_acc[n][0][1][1];
          float y = patches_acc[n][1][1][1];
          float d = patches_acc[n][2][1][1];
          center_acc[n][0] = x;
          center_acc[n][1] = y;
          center_acc[n][2] = d;
      }

      // Save both poses and extracted patch centers to header
      save_tensor_as_c_pair(poses, "golden_poses", out_path);
      save_tensor_as_c_pair(patches_center, "golden_patches", out_path);

      //print first 10 patches
      //std::cout << "First 10 patches after update:" << std::endl;
      //for (int i = 0; i < 10 && i < patches.size(0); i++) {
      //    std::cout << "Patch " << i << ": "
      //              << patches_acc[i][0][1][1] << ", "
      //              << patches_acc[i][1][1][1] << ", "
      //              << patches_acc[i][2][1][1] << std::endl;
      //}

      //print first 10 centered patches
      //std::cout << "First 10 centered patches after update:" << std::endl;
      //for (int i = 0; i < 10 && i < patches_center.size(0); i++) {
      //    std::cout << "Patch " << i << ": "
      //              << center_acc[i][0] << ", "
      //              << center_acc[i][1] << ", "
      //              << center_acc[i][2] << std::endl;
//
      //}
