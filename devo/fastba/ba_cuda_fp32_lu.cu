       ...
       for (int i=0; i<6; i++) {
        for (int j=0; j<6; j++) {
          if (ix >= 0)
          {
            if (tid == 150)
            {
                float val_before = static_cast<float>(local_B[6*ix+i][6*ix+j]);
                float delta = w * Ji[i] * Ji[j];

                //printf("[Thread %d] Before: %.10f | Delta: %.10f\n", threadIdx.x, val_before, delta);
            }
            atomicAdd(&local_B[6*ix+i][6*ix+j],  w * Ji[i] * Ji[j]);
            if (tid == 150)
            {
                //printf("[Thread %d] After: %.10f\n", threadIdx.x, (float)(local_B[6*ix+i][6*ix+j]));
            }
          }
          ...
          {
            if (tid == 150) {
                float val_before = static_cast<float>(local_B[6*jx+i][6*jx+j]);
                float delta = w * Jj[i] * Jj[j];

                //printf("[Thread %d] Before: %.10f | Delta: %.10f\n", threadIdx.x, val_before, delta);
            }
            atomicAdd(&local_B[6*jx+i][6*jx+j],  w * Jj[i] * Jj[j]);
            if (tid == 150) {
                //printf("[Thread %d] After: %.10f\n", threadIdx.x, (float)(local_B[6*jx+i][6*jx+j]));
            }
          }
          if (ix >= 0 && jx >= 0) {
            if (tid == 150) {
                float val_before = static_cast<float>(local_B[6*ix+i][6*jx+j]);
                float delta = -w * Ji[i] * Jj[j];
            ...
            if (tid == 150) {
                //printf("[Thread %d] After: %.10f\n", threadIdx.x, (float)(local_B[6*ix+i][6*jx+j]));
            }
          }
        }
      }
    ...
    const int t0, const int t1, const int iterations)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    if (t1 - t0 == 0) {
      ...
      //print_2d_tensor_to_file(S, "S_matrix_before_reg_fp32_GT.txt", itr);

      torch::Tensor I = torch::eye(6*N, opts);
      S += I * (1e-4 * S + 1.0);
      //print_tensor_stats(S, "S after regularization");

      // Convert to FP32 for numerical stability
      if (itr == 0) std::remove("S_fp32_lu.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(S, "S_fp32_lu.txt", itr);
      ...
      if (itr == 0) std::remove("LU_fp32_lu.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(LU, "LU_fp32_lu.txt", itr);
