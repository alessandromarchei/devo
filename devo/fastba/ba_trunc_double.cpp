...
std::vector<torch::Tensor> ba_trunc_double(
    torch::Tensor poses,
    torch::Tensor patches,
    torch::Tensor intrinsics,
    torch::Tensor target,
    ...
    torch::Tensor kk,
    int t0, int t1, int iterations, int decimal_places = 0) {
  return cuda_ba_trunc_double(poses, patches, intrinsics, target, weight, lmbda, ii, jj, kk, t0, t1, iterations, decimal_places);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &ba_trunc_double, "BA forward operator with logging");
