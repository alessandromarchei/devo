...
__global__ void round_tensor_kernel_anyshape(float* data, int64_t numel, const float factor) {
...
void round_tensor(torch::Tensor& tensor, int decimal_places) {
    float factor = powf(10.0f, (float)decimal_places);
    ...
    TORCH_CHECK(tensor.scalar_type() == torch::kFloat32, "round_tensor only supports float32 tensors.");

    torch::Tensor tensor_flat = tensor.view(-1);

    float* data_ptr = tensor_flat.data_ptr<float>();
...
std::vector<torch::Tensor> cuda_ba_trunc(
    torch::Tensor poses,
    torch::Tensor patches,
    torch::Tensor intrinsics,
    torch::Tensor target,
