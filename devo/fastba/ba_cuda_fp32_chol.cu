    ...
    const int t0, const int t1, const int iterations)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    if (t1 - t0 == 0) {
      ...
      if (itr == 0) std::remove("S_fp32_chol.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(S, "S_fp32_chol.txt", itr);
      torch::Tensor L = torch::linalg::cholesky(S);
      torch::Tensor dX = torch::cholesky_solve(y, L);

      if (itr == 0) std::remove("L_fp32_chol.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(L, "L_fp32_chol.txt", itr);
