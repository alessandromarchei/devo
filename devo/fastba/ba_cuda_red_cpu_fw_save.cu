    ...
    const int t0, const int t1, const int iterations)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    std::cout << "saving tensors for iteration " << itr << std::endl;
    torch::save(B.detach().cpu(), "B_" + std::to_string(itr) + "_fw.pt");

    torch::save(E.detach().cpu(), "E_" + std::to_string(itr) + "_fw.pt");

    torch::save(C.detach().cpu(), "C_" + std::to_string(itr) + "_fw.pt");

    torch::save(v.detach().cpu(), "v_" + std::to_string(itr) + "_fw.pt");

    torch::save(u.detach().cpu(), "u_" + std::to_string(itr) + "_fw.pt");
