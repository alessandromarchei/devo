...
B = matrix(60, 60)  # Pose Hessian
E = matrix(60, 700)   # Pose-patch coupling
C = matrix(700)       # Patch Hessian diagonal
v = matrix(60)      # Pose residual vector
u = matrix(700)       # Patch residual vector

for each edge (ii, jj, kk):

    residual, Ji, Jj, Jz = compute_residual_and_jacobians(pose[ii], pose[jj], patch[kk], target)
