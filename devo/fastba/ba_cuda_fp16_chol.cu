    ...
    const int t0, const int t1, const int iterations)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    if (t1 - t0 == 0) {
      ...
      if (itr == 0) std::remove("S_fp16_chol.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(S, "S_fp16_chol.txt", itr);

      torch::Tensor L = torch::linalg::cholesky(S);

      //clamp
      L = torch::clamp(L, -65504.0f, 65504.0f);
      torch::Tensor dX = torch::cholesky_solve(y, L);

      //clamp
      dX = torch::clamp(dX, -65504.0f, 65504.0f);

      if (itr == 0) std::remove("L_fp16_chol.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(L, "L_fp16_chol.txt", itr);
