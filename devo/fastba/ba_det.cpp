...
std::vector<torch::Tensor> ba_det(
    torch::Tensor poses,
    torch::Tensor patches,
    torch::Tensor intrinsics,
    torch::Tensor target,
    ...
    torch::Tensor kk,
    int t0, int t1, int iterations) {
  return cuda_ba_det(poses, patches, intrinsics, target, weight, lmbda, ii, jj, kk, t0, t1, iterations);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &ba_det, "BA forward operator");
