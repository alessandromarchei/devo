    ...
    const int t0, const int t1, const int iterations)
{
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    std::cout << "B is tensor? " << std::boolalpha << B.is_cuda() << ", dtype: " << B.dtype() << ", shape: " << B.sizes() << std::endl;
    std::cout << "E is tensor? " << std::boolalpha << E.is_cuda() << ", dtype: " << E.dtype() << ", shape: " << E.sizes() << std::endl;
    std::cout << "C is tensor? " << std::boolalpha << C.is_cuda() << ", dtype: " << C.dtype() << ", shape: " << C.sizes() << std::endl;
    std::cout << "v is tensor? " << std::boolalpha << v.is_cuda() << ", dtype: " << v.dtype() << ", shape: " << v.sizes() << std::endl;
    std::cout << "u is tensor? " << std::boolalpha << u.is_cuda() << ", dtype: " << u.dtype() << ", shape: " << u.sizes() << std::endl;

    torch::save(B.detach().cpu(), "B_" + std::to_string(itr) + "_bw.pt");

    torch::save(E.detach().cpu(), "E_" + std::to_string(itr) + "_bw.pt");

    torch::save(C.detach().cpu(), "C_" + std::to_string(itr) + "_bw.pt");

    torch::save(v.detach().cpu(), "v_" + std::to_string(itr) + "_bw.pt");

    torch::save(u.detach().cpu(), "u_" + std::to_string(itr) + "_bw.pt");
