...
__device__ void
actSO3(const at::Half *q, const at::Half *X, at::Half *Y) {
  at::Half uv[3];
...
__device__  void
actSE3(const at::Half *t, const at::Half *q, const at::Half *X, at::Half *Y) {
  actSO3(q, X, Y);
  Y[3] = X[3];
  Y[0] += X[3] * t[0];
  Y[1] += X[3] * t[1];
...
__device__ void
adjSE3(const at::Half *t, const at::Half *q, const at::Half *X, at::Half *Y) {
  at::Half qinv[4] = {-q[0], -q[1], -q[2], q[3]};
  actSO3(qinv, &X[0], &Y[0]);
  actSO3(qinv, &X[3], &Y[3]);
...
__device__ void 
relSE3(const at::Half *ti, const at::Half *qi, const at::Half *tj, const at::Half *qj, at::Half *tij, at::Half *qij) {
...
__device__ void
expSO3(const at::Half *phi, at::Half* q) {
  // SO3 exponential map
  at::Half theta_sq = phi[0]*phi[0] + phi[1]*phi[1] + phi[2]*phi[2];
  //check_nan_inf(&theta_sq, 1, "theta_sq", "expSO3");
  at::Half theta_p4 = theta_sq * theta_sq;
  //check_nan_inf(&theta_p4, 1, "theta_p4", "expSO3");

  at::Half theta = sqrtf(theta_sq);
...
__device__ void
crossInplace(const at::Half* a, at::Half *b) {
  at::Half x[3] = {
    a[1]*b[2] - a[2]*b[1],
    a[2]*b[0] - a[0]*b[2],
    a[0]*b[1] - a[1]*b[0], 
...
__device__ void
expSE3(const at::Half *xi, at::Half* t, at::Half* q) {
  // SE3 exponential map

  expSO3(xi + 3, q);
  at::Half tau[3] = {xi[0], xi[1], xi[2]};
  at::Half phi[3] = {xi[3], xi[4], xi[5]};
  ...
  if (theta > 1e-4) {
    at::Half a = (1 - cosf(theta)) / theta_sq;
    crossInplace(phi, tau);
    t[0] += a * tau[0];
    t[1] += a * tau[1];
    t[2] += a * tau[2];

    at::Half b = (theta - sinf(theta)) / (theta * theta_sq);
    crossInplace(phi, tau);
    t[0] += b * tau[0];
    t[1] += b * tau[1];
    t[2] += b * tau[2];
...
__device__ void
retrSE3(const at::Half *xi, const at::Half* t, const at::Half* q, at::Half* t1, at::Half* q1) {
  // retraction on SE3 manifold

  at::Half dt[3] = {0, 0, 0};
  at::Half dq[4] = {0, 0, 0, 1};
...
__global__ void pose_retr_kernel(const int t0, const int t1,
    torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> poses,
    torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> update)
{
  GPU_1D_KERNEL_LOOP(i, t1 - t0) {
    const at::Half t = t0 + i;
    at::Half t1[3], t0[3] = { poses[t][0], poses[t][1], poses[t][2] };
    at::Half q1[4], q0[4] = { poses[t][3], poses[t][4], poses[t][5], poses[t][6] };
...
__global__ void patch_retr_kernel(
    ...
    torch::PackedTensorAccessor32<at::Half,4,torch::RestrictPtrTraits> patches,
    torch::PackedTensorAccessor32<at::Half,1,torch::RestrictPtrTraits> update)
{
  GPU_1D_KERNEL_LOOP(n, index.size(0)) {
    const int p = patches.size(2);
    const int ix = index[n];
  
    at::Half d = patches[ix][2][0][0];
    d = d + update[n];
    d = (d > at::Half(20)) ? at::Half(1.0) : d;
    d = max(d, at::Half(1e-4));
...
__global__ void reprojection_residuals_and_hessian(
    const torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> poses,
    const torch::PackedTensorAccessor32<at::Half,4,torch::RestrictPtrTraits> patches,
    const torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> intrinsics,
    const torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> target,
    const torch::PackedTensorAccessor32<at::Half,2,torch::RestrictPtrTraits> weight,
    const torch::PackedTensorAccessor32<at::Half,1,torch::RestrictPtrTraits> lmbda,
    ...
    torch::PackedTensorAccessor64<at::Half,3,torch::RestrictPtrTraits> B,
    torch::PackedTensorAccessor64<at::Half,3,torch::RestrictPtrTraits> E,
    torch::PackedTensorAccessor64<at::Half,2,torch::RestrictPtrTraits> C,
    torch::PackedTensorAccessor64<at::Half,2,torch::RestrictPtrTraits> v,
    torch::PackedTensorAccessor64<at::Half,2,torch::RestrictPtrTraits> u,
    const int t0)
{
    ...
    __shared__ at::Half fx, fy, cx, cy;
    if (threadIdx.x == 0) {
        fx = intrinsics[0][0];
        fy = intrinsics[0][1];
        cx = intrinsics[0][2];
    ...
    GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
      ...
      at::Half ti[3] = { poses[ix][0], poses[ix][1], poses[ix][2] };
      at::Half tj[3] = { poses[jx][0], poses[jx][1], poses[jx][2] };
      at::Half qi[4] = { poses[ix][3], poses[ix][4], poses[ix][5], poses[ix][6] };
      at::Half qj[4] = { poses[jx][3], poses[jx][4], poses[jx][5], poses[jx][6] };
      ...
      const at::Half X = Xj[0];
      const at::Half Y = Xj[1];
      const at::Half Z = Xj[2];
      const at::Half W = Xj[3];
      ...
      const at::Half d = (Z >= 0.2) ? 1.0 / Z : 0.0; 
      const at::Half d2 = d * d;

      //CHECK_NAN_INF("d", d);
      //CHECK_NAN_INF("d2", d2);

      const at::Half x1 = fx * (X / Z) + cx;
      const at::Half y1 = fy * (Y / Z) + cy;

      const at::Half rx = target[n][0] - x1;
      const at::Half ry = target[n][1] - y1;
      ...
      const at::Half mask = in_bounds ? 1.0 : 0.0;
    ...
    {
      const at::Half r = target[n][0] - x1;
      const at::Half w = mask * weight[n][0];

      //CHECK_NAN_INF("r", r);
      //CHECK_NAN_INF("w", w);


      at::Half Jz = fx * (tij[0] * d - tij[2] * (X * d2));
      at::Half Ji[6], Jj[6] = {fx*W*d, 0, fx*-X*W*d2, fx*-X*Y*d2, fx*(1+X*X*d2), fx*-Y*d};
      ...
      for (int i=0; i<6; i++) {
        for (int j=0; j<6; j++) {
          if (ix >= 0)
          {
            ...
            atomicAdd(&local_B[6*ix+i][6*ix+j],  at::Half(w * Ji[i] * Ji[j]));
            //check if new value added is Inf, in that case cast it to the higher half precision value
            at::Half& ref = local_B[6*ix+i][6*ix+j];
            ref = at::Half(fminf(fmaxf((float)ref, -65504.0f), 65504.0f));
          ...
          if (jx >= 0)
          {
            ...
            atomicAdd(&local_B[6*jx+i][6*jx+j],  at::Half(w * Jj[i] * Jj[j]));
            at::Half& ref = local_B[6*jx+i][6*jx+j];
            ref = at::Half(fminf(fmaxf((float)ref, -65504.0f), 65504.0f));
          ...
          if (ix >= 0 && jx >= 0) {
            ...
            atomicAdd(&local_B[6*ix+i][6*jx+j], at::Half(-w * Ji[i] * Jj[j]));
            at::Half& ref = local_B[6*ix+i][6*jx+j];
            ref = at::Half(fminf(fmaxf((float)ref, -65504.0f), 65504.0f));
            //check if new value added is
            // if (isinf(local_B[6*ix+i][6*jx+j])) {
            //     local_B[6*ix+i][6*jx+j] = at::Half(65504.0f);
            // }
            atomicAdd(&local_B[6*jx+i][6*ix+j], at::Half(-w * Jj[i] * Ji[j]));
            at::Half& ref2 = local_B[6*jx+i][6*ix+j];
            ref2 = at::Half(fminf(fmaxf((float)ref2, -65504.0f), 65504.0f));
            //check if new value
      ...
      for (int i=0; i<6; i++) {
        if (ix >= 0)
        {
          atomicAdd(&local_E[6*ix+i][k], at::Half(-w * Jz * Ji[i]));
          at::Half& ref = local_E[6*ix+i][k];
          ref = at::Half(fminf(fmaxf((float)ref, -65504.0f), 65504.0f));
          //check if new value added is
          // if(isinf(local_E[6*ix+i][k])) {
          //   local_E[6*ix+i][k] = at::Half(65504.0f);
          // }
        }
        if (jx >= 0)
        {
          atomicAdd(&local_E[6*jx+i][k],  at::Half(w * Jz * Jj[i]));
          at::Half& ref2 = local_E[6*jx+i][k];
          ref2 = at::Half(fminf(fmaxf((float)ref2, -65504.0f), 65504.0f));
          //check if new value
          // if(isinf(local_E[6*jx+i][k])) {
          //   local_E[6*jx+i][k] = at::Half(65504.0f);
          // }
        }
      ...
      for (int i=0; i<6; i++) {
        if (ix >= 0)
        {
          atomicAdd(&local_v[6*ix+i], at::Half(-w * r * Ji[i]));
          at::Half& ref = local_v[6*ix+i];
          ref = at::Half(fminf(fmaxf((float)ref, -65504.0f), 65504.0f));
        // if (isinf(local_v[6*ix+i])) {
        //     local_v[6*ix+i] = at::Half(65504.0f);
        // }
        }
        if (jx >= 0)
        {
          atomicAdd(&local_v[6*jx+i],  at::Half(w * r * Jj[i]));
          at::Half& ref2 = local_v[6*jx+i];
          ref2 = at::Half(fminf(fmaxf((float)ref2, -65504.0f), 65504.0f));
          //check if new value
          // if (isinf(local_v[6*jx+i])) {
          //     local_v[6*jx+i] = at::Half(65504.0f);
          // }
        }

      }

      atomicAdd(&local_C[k], at::Half(w * Jz * Jz));
      at::Half& ref3 = local_C[k];
      ref3 = at::Half(fminf(fmaxf((float)ref3, -65504.0f), 65504.0f));
      //check if new value
      // if (isinf(local_C[k])) {
      //     local_C[k] = at::Half(65504.0f);
      // }
      atomicAdd(&local_u[k], at::Half(w *  r * Jz));
      at::Half& ref4 = local_u[k];
      ref4 = at::Half(fminf(fmaxf((float)ref4, -65504.0f), 65504.0f));
      //check if new value
      // if (isinf(local_u[k])) {
      //     local_u[k] = at::Half(65504.0f);
      // }
    }

    {
      const at::Half r = target[n][1] - y1;
      const at::Half w = mask * weight[n][1];
      
      at::Half Jz = fy * (tij[1] * d - tij[2] * (Y * d2));
      at::Half Ji[6], Jj[6] = {0, fy*W*d, fy*-Y*W*d2, fy*(-1-Y*Y*d2), fy*(X*Y*d2), fy*X*d};
    ...
    const int t0, const int t1, const int iterations)
{
  ...
  // // Cast to half
  // poses = poses.to(torch::kHalf);
  // patches = patches.to(torch::kHalf);
  // intrinsics = intrinsics.to(torch::kHalf);
  // target = target.to(torch::kHalf);
  // weight = weight.to(torch::kHalf);
  // lmbda = lmbda.to(torch::kHalf);
  ...
  auto opts = torch::TensorOptions()
    .dtype(torch::kHalf).device(torch::kCUDA);
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    reprojection_residuals_and_hessian<<<NUM_BLOCKS(ii.size(0)), NUM_THREADS_PER_BLOCK>>>(
      poses.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>(),
      patches.packed_accessor32<at::Half,4,torch::RestrictPtrTraits>(),
      intrinsics.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>(),
      target.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>(),
      weight.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>(),
      lmbda.packed_accessor32<at::Half,1,torch::RestrictPtrTraits>(),
      ...
      B.packed_accessor64<at::Half,3,torch::RestrictPtrTraits>(),
      E.packed_accessor64<at::Half,3,torch::RestrictPtrTraits>(),
      C.packed_accessor64<at::Half,2,torch::RestrictPtrTraits>(),
      v.packed_accessor64<at::Half,2,torch::RestrictPtrTraits>(),
      u.packed_accessor64<at::Half,2,torch::RestrictPtrTraits>(),
      t0);
    ...
    B = torch::clamp(B, -65504.0f, 65504.0f);
    E = torch::clamp(E, -65504.0f, 65504.0f);
    C = torch::clamp(C, -65504.0f, 65504.0f);
    v = torch::clamp(v, -65504.0f, 65504.0f);
    u = torch::clamp(u, -65504.0f, 65504.0f);
    ...
    Q = Q.to(torch::kHalf);
    //clamp
    Q = torch::clamp(Q, -65504.0f, 65504.0f);
    ...
    if (t1 - t0 == 0) {
      ...
      patch_retr_kernel<<<NUM_BLOCKS(M), NUM_THREADS_PER_BLOCK>>>(
        ...
        patches.packed_accessor32<at::Half,4,torch::RestrictPtrTraits>(),
        dZ.packed_accessor32<at::Half,1,torch::RestrictPtrTraits>());
      ...
      torch::Tensor EQ = E * Q;
      EQ = EQ.to(torch::kHalf);
      //clamp
      EQ = torch::clamp(EQ, -65504.0f, 65504.0f);

      //check_tensor_nan_inf(EQ.cpu(), "EQ");
      //print_tensor_stats(EQ, "EQ");
      torch::Tensor Et = torch::transpose(E, 0, 1);
      Et = Et.to(torch::kHalf);
      torch::Tensor Qt = torch::transpose(Q, 0, 1);
      Qt = Qt.to(torch::kHalf);
      ...
      S = S.to(torch::kHalf);
      //clamp
      S = torch::clamp(S, -65504.0f, 65504.0f);
      ...
      y = y.to(torch::kHalf);
      //clamp
      y = torch::clamp(y, -65504.0f, 65504.0f);
      ...
      //clamp all values that are greater than 65504 to 65504
      S = torch::clamp(S, -65504.0f, 65504.0f);
      ...
      // Convert to FP32 for numerical stability
      if (itr == 0) std::remove("S_fp16_lu.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(S, "S_fp16_lu.txt", itr);
      auto result = torch::linalg_lu_factor(S);  // ✅ correct function for LU solve
      torch::Tensor LU = std::get<0>(result);
      torch::Tensor pivots = std::get<1>(result);

      if (itr == 0) std::remove("LU_fp16_lu.txt");
      //if (itr == 0) print_2d
      if (itr == 0) print_2d_tensor_to_file(LU, "LU_fp16_lu.txt", itr);

      // Solve the system Sx = y using LU and pivots
      torch::Tensor dX = torch::linalg_lu_solve(LU, pivots, y);  // ✅ correct usage


      //clamp
      dX = torch::clamp(dX, -65504.0f, 65504.0f);

      dX = dX.to(torch::kHalf);

      //print_tensor_stats(dX, "dX");
      torch::Tensor dZ = Qt * (u - torch::matmul(Et, dX));
      //clamp
      dZ = torch::clamp(dZ, -65504.0f, 65504.0f);
      //check_tensor_nan_inf(dZ.cpu(), "dZ
      dZ = dZ.to(torch::kHalf);
      ...
      pose_retr_kernel<<<NUM_BLOCKS(N), NUM_THREADS_PER_BLOCK>>>(t0, t1,
          poses.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>(),
          dX.packed_accessor32<at::Half,2,torch::RestrictPtrTraits>());
