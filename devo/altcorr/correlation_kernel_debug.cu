...
std::vector<torch::Tensor> corr_cuda_forward(
  torch::Tensor fmap1,    // [1, BUF * NPATCHES, MATCH_FEAT_DIM, 3, 3]
  torch::Tensor fmap2,    // [1, BUF, MATCH_FEAT_DIM, H/4, W/4]
  torch::Tensor coords,   // [1, NPATCHES, 2, 3, 3]
  torch::Tensor ii,   // [NPATCHES] 
  torch::Tensor jj,   // [NPATCHES]
  int radius)         // 3
{
  const int B = coords.size(0);
  const int M = coords.size(1);
  ...
  std::cout << "corr_cuda_forward: B=" << B << ", M=" << M << ", H=" << H << ", W=" << W << ", D=" << D << std::endl;

  //print on screen the shapes of the tensors
  std::cout << "fmap1 shape: " << fmap1.sizes() << std::endl;
  std::cout << "fmap2 shape: " << fmap2.sizes() << std::endl;
  std::cout << "coords shape: " << coords.sizes() << std::endl;
  std::cout << "ii shape: " << ii.sizes() << std::endl;
  std::cout << "jj shape: " << jj.sizes() << std::endl;
  std::cout << "corr shape: " << corr.sizes() << std::endl;
