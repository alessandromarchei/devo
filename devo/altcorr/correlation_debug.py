...
class CorrLayer(torch.autograd.Function):
    ...
    def forward(ctx, fmap1, fmap2, coords, ii, jj, radius, dropout):
        ...
        ctx.radius = radius
        ctx.dropout = dropout
        corr, = cuda_corr_debug.forward(fmap1, fmap2, coords, ii, jj, radius)
    ...
    def backward(ctx, grad):
        ...
        fmap1_grad, fmap2_grad = \
            cuda_corr_debug.backward(fmap1, fmap2, coords, ii, jj, grad, ctx.radius)
...
class PatchLayer(torch.autograd.Function):
    ...
    def forward(ctx, net, coords, radius):
        ...
        patches, = cuda_corr_debug.patchify_forward(net, coords, radius)
        return patches
    ...
    def backward(ctx, grad):
        ...
        grad, = cuda_corr_debug.patchify_backward(net, coords, grad, ctx.radius)
...
def corr_debug(fmap1, fmap2, coords, ii, jj, radius=1, dropout=1):
