...
class PatchSelector():
    def __init__(self, method="multi", grid=False, **kwargs) -> None:
        ...
        self.BORDER_SUPPRESSION = kwargs.get("border_suppression_size", 0)
        self.NMS_RADIUS = kwargs.get("nms_radius", 11)
    ...
    def _multi(self, scores, patches_per_image):
        ...
        if self.grid:
            ...
            if not torch.all(torch.isfinite(scores_t)):
                #error here, exit with a None value for the coordinates and use the evtopk method instead, on the outer function
                print("Error in the scores tensor, returning None")
                return None, None
            
            
            #assert torch.all(torch.isfinite(scores_t)), f"Found NaN or Inf in scores_t: {scores_t}"
            #assert torch.all(scores_t >= 0), f"Found negative values in scores_t: {scores_t.min()}"
            #assert torch.all(scores_t.sum(dim=1) > 0), "Some rows in scores_t sum to zero"
