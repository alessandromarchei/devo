...
class DEVO:
    def __init__(self, cfg, network, evs=False, ht=480, wd=640, viz=False, viz_flow=False, dim_inet=384, dim_fnet=128, dim=32, profiling=False, **kwargs):
        self.cfg = cfg
        self.evs = evs
    ...
    def update(self):
        ...
        with autocast(enabled=True):
            ...
            with Timer("other", enabled=self.enable_timing):
                    ...
                    #RUNNING INFERENCE OF THE UPDATE NETWORK HERE
                #INFERENCE of the PATCHIFIER function
                if self.profiling is not False and not self.update_done and self.n > 20:
                    with open("profile/update_summary.txt", "w") as f:
                        summary_str = summary(self.network.update, 
                            input_data=(self.net, ctx, corr, self.ii, self.jj, self.kk),  # Adjust to match expected input size
                            device="cuda",
                            verbose=2,
                            col_names=["input_size", "output_size", "num_params", "params_percent","mult_adds","kernel_size" ]
                        )
                        f.write("Summary of the Update Network\n")
    ...
    def __call__(self, tstamp, image, intrinsics, scale=1.0):
        ...
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            ...
            #INFERENCE of the PATCHIFIER function
            if self.profiling is not False and not self.patchify_done and self.n > 20:
                with open("profile/patchify_summary.txt", "w") as f:
                    summary_str = summary(self.network.patchify, 
                        input_data=(image),  # Adjust to match expected input size
                        device="cuda",
                        verbose=2,
                        depth=2,
                        col_names=["input_size", "output_size", "num_params", "params_percent","mult_adds","kernel_size" ],
                    )
                    f.write("Summary of the Patchify Network\n")
                    f.write(str(summary_str))
                #summary(self.network.patchify, input_data=(input_,), device="cuda",verbose=2,depth=5, col_names=["input_size", "output_size", "num_params", "mult_adds"])

                self.patchify_done = True
        ...
        if self.profiling and self.patchify_done and self.update_done:
            sys.exit(0)
        return