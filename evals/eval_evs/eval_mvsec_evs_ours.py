...
def evaluate(config, args, net, train_step=None, datapath="", split_file=None, 
             trials=1, stride=1, plot=False, save=False, return_figure=False, viz=False, timing=False, side='left', viz_flow=False, use_event_stack=False):
    ...
    for i, scene in enumerate(scenes):
        ...
        for trial in range(trials):
            ...
            traj_est, tstamps, flowdata = run_voxel(datapath_val, config, net, viz=viz, 
                                          iterator=mvsec_evs_iterator(datapath_val, side=side, stride=stride, timing=timing, H=H, W=W,use_event_stack=use_event_stack),
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--use_event_stack', action="store_true", help="Use voxel grid for evaluation")
    ...
    val_results, val_figures = evaluate(cfg, args, args.weights, datapath=args.datapath, split_file=args.val_split, trials=args.trials, \
                       plot=args.plot, save=args.save_trajectory, return_figure=args.return_figs, viz=args.viz,timing=args.timing, \
                        stride=args.stride, side=args.side, viz_flow=args.viz_flow, use_event_stack=args.use_event_stack)
